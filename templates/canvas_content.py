Higher Education Analytics - Data Validation Notebook Template
...
Purpose
Validate generated data for Higher Ed analytics demos before uploading to PostgreSQL or Salesforce. Supports multiple business units: Recruitment & Admissions, Registrar & Enrollment Management, Student Success, Corporate Engagement, and Advancement.
Horizontal Rule
...
BUSINESS_UNIT = "Recruitment & Admissions"  # Options: Recruitment & Admissions, Registrar & Enrollment Management, Student Success, Corporate Engagement, Advancement
...
BU_CONFIG = {
    "Recruitment & Admissions": {
        ...
        "primary_metrics": ["Applications", "Acceptances", "Yield Rate", "Conversion Rate"],
        "pipeline_stages": ["Inquiry", "Application Started", "Application Submitted", "Accepted", "Enrolled"]
    },
    "Registrar & Enrollment Management": {
        "constituents": ["Students", "Advisors", "Programs", "Courses"],
        "primary_metrics": ["Enrollment Count", "Credit Hours", "Full-Time Equivalent", "Course Fill Rate"],
        "pipeline_stages": ["Enrolled", "Active", "On Leave", "Withdrawn", "Graduated"]
    },
    "Student Success": {
        "constituents": ["Students", "Advisors", "Support Services"],
        "primary_metrics": ["Retention Rate", "Graduation Rate", "GPA", "Engagement Score"],
        "pipeline_stages": ["At Risk", "Improving", "On Track", "Excelling"]
    },
    "Corporate Engagement": {
        "constituents": ["Sponsors", "Researchers", "Projects"],
        "primary_metrics": ["Funding Amount", "Active Grants", "Research Output", "Partnership Count"],
        "pipeline_stages": ["Prospecting", "Proposal", "Awarded", "Active", "Completed"]
    },
    "Advancement": {
        "constituents": ["Donors", "Gift Officers", "Campaigns"],
        "primary_metrics": ["Gift Amount", "Donor Count", "Pledge Amount", "Campaign Progress"],
...
#Direct upsert to PostgreSQL with post-script#
EXPORT_TO_POSTGRES = False  # Set to True to execute

if EXPORT_TO_POSTGRES:
    print("=== POSTGRESQL EXPORT ===")
    
    TABLE_NAME = "demo_data_table"  # Customize
    
    #Initialize loader#
    pg_loader = PostgresLoader(config)
    
    #Upsert data#
    try:
        pg_loader.upsert_table(df, TABLE_NAME)
        print(f"✅ Upserted {len(df):,} records to {TABLE_NAME}")
        
        #Execute post-script#
        pg_loader.register_table(TABLE_NAME, "harold.thomas@salesforce.com")
        print(f"✅ Registered table in q_table_reference")
        
    except Exception as e:
        print(f"❌ Error: {e}")
else:
    print("ℹ️ PostgreSQL export disabled. Set EXPORT_TO_POSTGRES = True to execute.")
9.2 Export to Salesforce
#Upsert to Salesforce with preview#
EXPORT_TO_SALESFORCE = False  # Set to True to execute
TARGET_ORG = "TableauNextEdu"  # From config
SALESFORCE_OBJECT = "Custom_Object__c"  # Customize

if EXPORT_TO_SALESFORCE:
    print("=== SALESFORCE EXPORT ===")
    
    #Initialize loader#
    sf_loader = SalesforceLoader(config, TARGET_ORG)
    
    #Schema mapping preview#
    print("\n📋 Schema Mapping Preview:")
    mapping = sf_loader.generate_mapping(df, SALESFORCE_OBJECT)
    for source_field, target_field in mapping.items():
        print(f"  {source_field} → {target_field}")
    
    #Validation preview#
    print(f"\n🔍 Records to be created/updated: {len(df):,}")
    print("\nSample records that will be upserted:")
    print(df.head(3))
    
    #Confirm before executing#
    CONFIRM_UPSERT = input("\nProceed with upsert? (yes/no): ")
    
    if CONFIRM_UPSERT.lower() == 'yes':
        try:
            result = sf_loader.upsert_records(df, SALESFORCE_OBJECT, mapping)
            print(f"✅ Successfully upserted {result['success_count']} records")
            if result['error_count'] > 0:
                print(f"⚠️ {result['error_count']} errors occurred")
        except Exception as e:
            print(f"❌ Error: {e}")
    else:
        print("ℹ️ Upsert cancelled")
else:
    print("ℹ️ Salesforce export disabled. Set EXPORT_TO_SALESFORCE = True to execute.")
